 ...
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export jd_data=' accesstoken '   ,多账号用 换行 或 @ 分割
 * 搜accesstocken填入变量
 */
 ...
 !(async () => {
	 let ckArr = await checkEnv(ckStr, "jd_data");
	 await tips(ckArr);
 ...
 async function signin() {
	 try {
		 let url = {
			 url: `${hostname}/api/user/daily/sign?exParams\u003dtrue`,
			 headers: {
				"Host": host,
				"accesstoken": ck[0]
			 },
  ...
  */
   async function jifen() {
	try {
		...
		if (result?.code == 200) {
			DoubleLog(`查询成功，当前有机值:${result.data}`);
			await wait(5);
		} else {
