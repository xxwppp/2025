...
"""
 作者:  临渊
 日期:  2025/7/5
 小程序:  社服益寿活动 (https://a.c1ns.cn/m6e7K) (首次进入点参与答题授权头像昵称)
 功能:  签到、问答、查询信息
 变量:  sfys_wxid_data (微信id) 多个账号用换行分割 
...
class AutoTask:
    ...
    def check_env(self):
        ...
        try:
            # 从环境变量获取cookie
            soy_wxid_data = os.getenv(f"sfys_wxid_data")
            if not soy_wxid_data:
                self.log("[检查环境变量] 没有找到环境变量sfys_wxid_data，请检查环境变量", level="error")
                return None
